...
## A13. Jira 인스턴스의 기본 타임존이 `Asia/Seoul`(KST)

- **가정:** Jira 서버에 설정된 기본 사용자 시간대가 `Asia/Seoul`이다.
- **조사 결과:** 사내 확인 필요 — 인스턴스별 관리자 설정이라 공개 문서로 확인할 수 없다.
- **근거:** `sync_issues.build_jql`이 워터마크를 `"YYYY-MM-DD HH:mm"` 형태의 오프셋 없는 JQL
  날짜 리터럴로 만든다(design.md §2.1, §5.2). JQL의 오프셋 없는 날짜/시각 리터럴은 Jira
  서버의 기본 타임존으로 해석된다는 것이 Atlassian JQL 문법의 일반 규칙이다 — 이 값이
  `Asia/Seoul`이 아니면, 저장 규약(KST)에 맞춰 만든 워터마크 문자열이 다른 타임존으로
  잘못 해석되어 증분 수집이 이슈를 놓치거나 불필요하게 재수집한다.
- **틀렸을 때 할 일:** `build_jql`이 인스턴스의 실제 기본 타임존 오프셋을 명시적으로
  붙이도록 고친다(예: `"updated >= \"2026-01-01 09:00\""` 대신 오프셋을 포함한 표현으로).
  `doctor --jira`가 자동으로 측정하지 않으므로, 사내 첫날 Jira 관리자 화면에서 직접
  확인해야 한다(onprem-runbook.md 6단계).
