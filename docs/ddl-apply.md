...
끝나면 `cli doctor --db`가 스키마 대조(DB7)까지 통과해야 한다. DB7은 이름만
비교하지 않는다 — 테이블·컬럼·**선언 폭**·인덱스·제약·뷰·시퀀스를 DDL 파일과
데이터 딕셔너리 사이에서 대조하고, 관측한 객체 수를 함께 찍는다 (파일 하나를
건너뛰면 여기서 드러난다). 폭 불일치는 코드의 `truncate` 상한
(`jira_dashboard/jira/models.py`)과 어긋난다는 뜻이므로 ORA-12899가 예정된
상태다 — DDL을 고쳐 맞춘다.
